...
/*
    for (Action action : actionsList) {
      for (int i = 0; i < action.getNumberOfParameters(); i++) {
        String parameterValue = action.getParameter(i);
        while (true) {
          String nextTag = findNextTag(parameterValue);
          if (nextTag == null) {
            break;
          }
          ...
          String value;
          try {
            value = event.getAttribute(nextTag);
          } catch (IllegalArgumentException e) {
            value = null;
          }

          if (value != null) {
            parameterValue.replaceAll(OPENING_TAG + nextTag + CLOSING_TAG, value);
          }
        }
      }
    }
*/
  ...
  private static String findNextTag(String text) {
    if (text == null) {
      return null;
    }

    int begTagPosition = text.indexOf(OPENING_TAG);
    int endTagPosition = text.indexOf(CLOSING_TAG);

    if (begTagPosition < 0 || endTagPosition < 0) {
      return null;
    }

    if (begTagPosition + 1 >= endTagPosition) {
      return null;
    }

    return text.substring(begTagPosition + 1, endTagPosition);
  }
