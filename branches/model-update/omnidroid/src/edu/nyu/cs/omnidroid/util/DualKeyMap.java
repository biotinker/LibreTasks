...
/**
 * <code>DualKeyMap</code> provides a Map like interface that supports dual keys.
 * A implementation of this interface should be wrapped around a <code>Map</code> implementation
 * like <code>HashMap</code>.
 * 
 ...
 *
 * @param <K1> is the type of the first key
 * @param <K2> is the type of the second key
 * @param <V> is the type of the third key
...
public interface DualKeyMap<K1,K2,V>{
     ...
     * If the map previously contained a mapping for this key pair, the old value 
     * is replaced by the specified value. 
     * 
	 * @param key1 is the first key
	 * @param key2 is the second key
	 * @param value is the value to be associate with the key pair
    ...
    /**
     * Dump all of the content of another DualKeyMap into this one.
     * The type parameter of the map of be dumped need to match this one
     * @param m is the DualKeyMap object needed to be dumped to the current one
     */
    ...
    /**
     * Return a set view that contains all of the key. These keys should be instances of
     * a class that implements the DualKeyMap.DualKey interface
    ...
    /**
     * A implementation of this map should be wrapped around a class implements Map
     * This method should just return that map object for convenience uses.
     * If this interface is implemented without wrapping a Map class (which could happen), this
     * methods should just return null
     * @return the internal map object wrapped internally, 
     * or null if this interface is not implemented around a map class.
     */
    public Map<? extends DualKey<K1,K2>,V> getMap();
    
    /**
     * A dual-key that combined two key object into a distinct key, which should be used as the
     * key of the internal map object.
     *
     * @param <T1> is the type of the first key
     * @param <T2> is the type of the  second key
     */
    @SuppressWarnings("unchecked")
	interface DualKey<T1,T2> extends Comparable{
    	 ...
    	 * @param other is the key pair to be compared with this key pair
