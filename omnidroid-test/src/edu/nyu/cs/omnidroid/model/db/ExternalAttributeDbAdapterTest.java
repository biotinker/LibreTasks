...
public class ExternalAttributeDbAdapterTest extends AndroidTestCase {
   ...
   * the parameters
   */
  private void assertCursorEquals(
      Cursor cursor, String attributeName, Long appID, Long dataTypeID) {
    assertEquals(cursor.getString(cursor.getColumnIndex(
        ExternalAttributeDbAdapter.KEY_EXTERNALATTRIBUTENAME)), attributeName);
    assertEquals(cursor.getInt(cursor.getColumnIndex(ExternalAttributeDbAdapter.KEY_APPID)), 
        appID.intValue());
